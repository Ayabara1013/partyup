...
export default function MyGames({className}) {
  ...
  const startFunc = async (game) => {
    if (await fbGmManagement.general.startGame(game.id)) {
      toast.success(`${game.name} has been started. Reddirecting...`)
      setGames()
      setTimeout(() => {
        push(`/games/play/${game.id}`);
      }, 1500)
    } else {
  ...
  return (
    (gmGames &&
      <div className={`${className} games-page page-wrapper flex flex-col gap-8`}>
        {gmGames && <MyGamesList gameList={gmGames.gameList} title={`GM Games`} type={'gm'} startFunc={startFunc}/>}
              ...
              return (
                <div className='games-page__games-list__item' key={index}>
                  <div className={`__item-header flex flex-wrap justify-start items-end gap-x-2`}>
                    ...
                    game.status === 'started'
                      ? <Link className='btn btn-xs btn-accent ml-auto font-semibold'
                        ...
                        Play</Link>
                      : <button className='btn btn-xs btn-accent ml-auto font-semibold'
                                onClick={() => {
                                  startFunc(game)
                                }}>
                        Start</button>
                  ...
                  <div className="flex gap-2">
                    ...
                    {(type === 'gm')
                      && <Link className='btn btn-xs btn-square btn-accent m-auto font-semibold'
                               href={`/games/my-games/gm/edit/${game.id}`}>edit</Link>}
                  </div>
                </div>
              )
            })
...
function PlayersList({className, game, playerCount, totalSeats}) {
  ...
  const RemainingSeats = () => {
    ...
    for (let i = playerCount; i < remainingSeats + playerCount; i++) {
      seats.push(
        <li key={i} className={`__players-list__item ${i >= totalSeats ? '--error' : '--empty'}`}>open</li>
      )
    }
    return seats;
  }
