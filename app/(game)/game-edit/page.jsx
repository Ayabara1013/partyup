...
import EditFormInput from "@/app/(game)/game-edit/_EditFormInput";
import EditFormActChapterInput from "@/app/(game)/game-edit/_EditFormActChapterInput";
...
export default function Page({ params }) {
  ...
  useEffect(() => {
    if (activeGames) {
      let gameId = accountLocalStorage.getEditGame();
      ...
      accountLocalStorage.setEditGame(null);
      push('/home');
    }
  }, [ activeGames ]);
  ...
  return (activeGames && game) && (
    <div className='home-page w-full h-full border'>
      ...
      <button className="btn w-full mt-5" onClick={updateGameOnClick}>Update Game!</button>
    </div>
  )
}