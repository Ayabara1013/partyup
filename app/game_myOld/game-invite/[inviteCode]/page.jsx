...
export default function Page({ params }) {
  ...
  const { user, activeGames } = useApplication();
  ...
  const getGameFromInvite = async () => {
    if (user && activeGames) {
      let gameDoc = await fbGenericManagement.player.findGameFromInviteCode(inviteCode);
      if (!gameDoc) {
  ...
  const joinGameOnClick = async () => {
    push('/home');
    await fbGenericManagement.player.acceptInviteLink(inviteCode, game.id);
