...
import ChatMessage from "@app/game_myOld/play/(chat-window)/(chatMessage)/chatMessage";
import ChatInput from "@app/games/play/[gameId]/(components)/chatInput";

export default function ChatWindow({ window, gameItems }) {
  let { game, messages, members, chatOptions } = gameItems
  ...
  return (
    <div className="border flex flex-col">
      ...
      {(!noInput) && <ChatInput id={window} game={game} window={window} chatOptions={chatOptions}/>}
    </div>
  )
}