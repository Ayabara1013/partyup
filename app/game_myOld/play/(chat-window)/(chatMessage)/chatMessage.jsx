...
import MessageContextMenu from "@app/game_myOld/play/(chat-window)/(contextMenu)/MessageContextMenu";
...
export default function ChatMessage({ game, message, members }) {
  ...
  const disableContextMenu = (e) => {
    ...
    let x = e.clientX;
    let y = e.clientY;
    let right = (e.clientX + 200 > window.innerWidth);
    right && (x = window.innerWidth - x);

    setContextMenu({
      options: <MessageContextMenu message={message} game={game}/>,
      style: {
        top: `${y}px`,
        ...right && { right: `${x}px` },
        ...!right && { left: `${x}px` }
