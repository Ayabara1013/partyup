        ...
        <div className={`invite-page tb1 h-full flex`}>
            <div className='border border-secondary flex flex-col m-auto p-4 gap-4 w-1/3 bg-neutral rounded-xl'>
                ...
                <div className='flex flex-col gap-4'>
                    ...
                    <SeatsList className={'flex-col-2'} targetGame={targetGame} seatState={seatState}
...
function SeatsList({className, targetGame, seatState, handleRemoveInvite}) {
    ...
    for (let i = 0; i < seatState.length; i++) {
        if (seatState[i] === 'empty') {
            ...
            seats.push(<FilledSlot user={user} playerCharacter={findCharacterByUserId(user.uid)} targetGame={targetGame}
                                   index={i} key={i}/>)
        }
    }
...
function FilledSlot({user, playerCharacter, targetGame, index}) {
    return (
        <div key={index}
             className='invite-panel__player-listing flex p-1 gap-2 bg-neutral-content bg-opacity-20 rounded-lg'>
...
function EmptySlot({targetGame, index, seatState, handleRemoveInvite}) {
    ...
    return (
        <div key={index}
             className={`${borderStyle} ${textStyle} invite-panel__player-listing flex p-1 gap-2 bg-neutral-content bg-opacity-20 rounded-lg`}>
            ...
            {seatState[index] ?
                <UndoInviteButton seatState={seatState} index={index} handleRemoveInvite={handleRemoveInvite}/> : null}
        </div>
    )
}
...
function UndoInviteButton({seatState, index, handleRemoveInvite}) {
    return (
        <button className={`${seatState[index] == false ? 'display-none' : ''} btn btn-ghost btn-xs hover:btn-accent`}
                onClick={() => handleRemoveInvite(index)}>x</button>
    )
}
